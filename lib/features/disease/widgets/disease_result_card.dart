...
class DiseaseResultCard extends ConsumerWidget {
  ...
  Future<void> _askGreenMindAI(BuildContext context, WidgetRef ref) async {
    ...
    final diseaseName = result.diseaseName.trim().isEmpty
        ? 'Unknown condition'
    ...
    final symptoms = result.symptoms.trim().isEmpty
        ? 'No symptom information available.'
        : result.symptoms.trim();

    final treatment = result.treatment.trim().isEmpty
        ? 'No treatment information available.'
        : result.treatment.trim();

    final prevention = result.prevention.trim().isEmpty
